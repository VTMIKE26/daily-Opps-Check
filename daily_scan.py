...
def score_opportunity(opp: Opportunity) -> Opportunity:
    ...
    if opp.score == 0:
        ...
        if is_eng and is_t1:
            ...
            opp.score_reasons = ["✓ Engagement event — Tier 1 LE/Security agency: watch for follow-on RFP"]
        elif is_eng:
            opp.score = 5
            opp.tier  = "🔵 Possible Fit"
            opp.score_reasons = ["✓ Engagement event — Federal agency engagement event"]
...
def build_industry_days_section(opps: list) -> str:
    """Separate section for any opportunity with 'industry day' in the title."""
    days = sorted(
        [o for o in opps
    ...
    rows = ""
    for o in days[:30]:
        link = (f'<a href="{o.url}" style="font-weight:700;font-size:14px;color:#1a5276;text-decoration:none;">{o.title[:120]}</a>'
                if o.url else f'<span style="font-weight:700;font-size:14px;color:#1a5276;">{o.title[:120]}</span>')
        rows += (
            '<div style="border:1px solid #d6eaf8;border-radius:6px;padding:12px;margin-bottom:8px;background:#eaf4fb;">'
            f'<div style="margin-bottom:4px;">{link}</div>'
            f'<div style="font-size:12px;color:#555;">🏛 {o.agency[:80]} &nbsp;·&nbsp; 📬 {o.posted_date[:10]}</div>'
            f'<div style="font-size:11px;color:#888;margin-top:2px;">Source: {o.source} &nbsp;·&nbsp; <a href="{o.url}" style="color:#1a5276;">View on SAM.gov</a></div>'
            '</div>'
        )
    return (
        '<div style="margin:20px 0 6px">'
        f'<h2 style="font-size:16px;color:#111;border-bottom:2px solid #d6eaf8;padding-bottom:5px;">📣 Industry Days ({len(days)})</h2>'
        '<p style="font-size:12px;color:#888;margin:0 0 10px;">All federal agency Industry Day notices — these precede formal solicitations.</p>'
        f'{rows}'
        '</div>'
    )
